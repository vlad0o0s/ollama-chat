...
- **Python 3.11+** (рекомендуется Python 3.13 для полной совместимости)
- **Node.js 16+** (протестировано с Node.js 22.20.0)
...
Все зависимости зафиксированы с точными версиями для обеспечения стабильности и воспроизводимости:

**Backend (Python):**
- FastAPI 0.110.0
- Uvicorn 0.27.1
- Pydantic 2.9.2 (совместимо с Python 3.13)
- SQLAlchemy 2.0.36 (совместимо с Python 3.13)
- Pillow 10.4.0 (совместимо с Python 3.13)
- И другие зависимости с точными версиями в `backend/requirements.txt`
...
- И другие зависимости с точными версиями в `frontend/package.json`
...
```mermaid
graph TD
    A[Пользователь вводит описание] --> B[GPT-OSS переводит в Natural Language промпт]
    B --> C[ComfyUI генерирует изображение]
    C --> D[Изображение сохраняется и отображается]
```
...
```mermaid
graph TD
    A[Пользователь загружает изображение] --> B[LLaVA анализирует изображение]
    B --> C[GPT-OSS генерирует Natural Language промпт]
    C --> D[GPT-OSS определяет настройки KSampler]
    D --> E[ComfyUI трансформирует изображение]
    E --> F[Результат сохраняется и отображается]
...
```mermaid
graph TD
    A[Запрос на использование GPU] --> B{Resource Manager}
    B --> C{Процесс активен?}
    C -->|Нет| D[Переключение процесса]
    C -->|Да| E[Проверка VRAM]
    D --> E
    E --> F{VRAM свободна?}
    F -->|Да| G[Блокировка GPU]
    F -->|Нет| H[Ожидание освобождения]
    H --> E
    G --> I[Выполнение задачи]
    I --> J[Освобождение GPU]
    J --> K{Восстановить Ollama?}
    K -->|Да| L[Переключение на Ollama]
    K -->|Нет| M[Завершение]
    L --> M
```
...
sequenceDiagram
    ...
    User->>API: Загрузить изображение + запрос
    API->>ResourceManager: Запрос GPU для Ollama
    ResourceManager->>Ollama: Переключение процесса
    Note over ResourceManager,Ollama: GPU заблокирован
    
    API->>LLaVA: Анализ изображения
    LLaVA-->>API: Описание изображения
    
    API->>GPT-OSS: Генерация промпта
    GPT-OSS-->>API: Natural Language промпт
    
    API->>GPT-OSS: Определение настроек KSampler
    GPT-OSS-->>API: Настройки (denoise, steps, etc.)
    
    Note over ResourceManager,Ollama: GPU освобожден
    
    API->>ResourceManager: Запрос GPU для ComfyUI
    ResourceManager->>ComfyUI: Переключение процесса
    API->>ComfyUI: Генерация изображения
    ComfyUI-->>API: Результат
    
    ResourceManager->>Ollama: Восстановление процесса
    API-->>User: Изображение готово
```
